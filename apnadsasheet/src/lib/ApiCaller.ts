...
class ApiCaller {
    ...
    private constructor(baseUrls: Record<string, string>, defaultBaseUrlKey: string) {
        this.baseUrls = baseUrls;
        this.defaultBaseUrlKey = defaultBaseUrlKey;

        if (!this.baseUrls[this.defaultBaseUrlKey]) {
            throw new Error(`Default base URL key "${this.defaultBaseUrlKey}" is missing in baseUrls.`);
        }
    }
    ...
    public async request<T = unknown>(
        ...
        try {
            ...
            if (abortController.signal.aborted) {
                return { data: null, error: "Request timed out or was aborted", status: null };
            }
