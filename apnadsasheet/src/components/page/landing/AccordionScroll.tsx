...
import {
  ...
  SkeletonProblemCard,
  SkeletonTopic,
} from "@/components/util/Skeletons.tsx";
...
const ProblemCard = ({ problem }: { problem: TProblem }) => {
  const { dispatch, bookmarkedProblem, loading, completedProblem } =
    useLandingContext();
    ...
    <Card className="p-4 hover:bg-gray-50 transition-colors duration-200 border-gray-200">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <Checkbox
            checked={completedProblem.includes(problem.id)}
            onClick={() =>
              dispatch({
...
const Chapter = ({ chapter }: { chapter: TChapter }) => {
  const { completedProblem, loading } = useLandingContext();
  const totalCompletedProblemForChapter = chapter.problems.filter((problem) =>
    completedProblem.includes(problem.id),
  );
  const totalProblems = chapter.problems.length;
  const progressPercentage =
    totalProblems > 0
      ? (totalCompletedProblemForChapter.length / totalProblems) * 100
      : 0;
...
const Topic = ({ topic }: { topic: TTopic }) => {
