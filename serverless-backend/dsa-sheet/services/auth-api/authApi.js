...
import {createSession, generateSessionToken, validateSessionToken} from "../../auth-utils/index.js";
import {v4} from "uuid";
...
router.post("/register", async (req, res) => {
    ...
    try {
        ...
        const existingUser = await db.select().from(userTable).where(eq(userTable.email,email)).limit(1)[0];
...
router.get("/problems/:userId",async (req,res)=>{
    ...
    const problems = await db.select().from(problemTable).where(eq(problemTable.userId, userId)).limit(1);
...
router.post("/problems/:userId", async (req, res) => {
    ...
    try {
        ...
        const existingProblem = (await db.select().from(problemTable).where(eq(problemTable.userId, userId)).limit(1))[0];
        console.log("Existing Problem: ", existingProblem);
        if (existingProblem) {
            // Append new problems to existing ones
            const updatedBookmarkedProblem = new Set([...existingProblem.bookmarkedProblem, ...bookmarkedProblem]);
            const updatedCompletedProblem = new Set([...existingProblem.completedProblem, ...completedProblem]);

            // Update the existing problem entry
            await db.update(problemTable)
                .set({
